...
contract BinPoolManagerTest is Test, GasSnapshot, BinTestHelper {
    ...
    function test_FuzzInitializePool(uint16 binStep) public {
        ...
        vm.expectEmit();
        emit Initialize(
            key.toId(), key.currency0, key.currency1, key.parameters.getFee(), binStep, IHooks(address(mockHooks))
        );
    ...
    function testGasSwapSingleBin() public {
        ...
        vm.expectEmit();
        emit Swap(
            key.toId(), address(binSwapHelper), -1 ether, (1 ether * 997) / 1000, activeId, key.parameters.getFee(), 0
        );
